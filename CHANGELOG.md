...
- INDIGO build wiring: `thirdparty/thirdparty.cmake` now mirrors the libindi pattern — default `ExternalProject_Add(indigo)` clones INDIGO from GitHub at a pinned SHA and builds libindigo at configure-time, so a fresh `git clone` of openastro-phd2 + `cmake -B build && make` produces a working binary with INDIGO support on macOS/Linux without the user having to pre-build anything. `BUILD_COMMAND` is scoped to `make init && make -C indigo_libs -j1 all` instead of the top-level `make all`: we're an INDIGO *client*, not a daemon, and the unscoped target builds hundreds of vendor drivers — several of which (Atik, ZWO ASI) ship Mac dylibs without enough Mach-O headerpad space for `install_name_tool` to rewrite, reliably failing the build mid-way through `indigo_drivers/`. `-j1` plus `MAKEFLAGS=` reset is required because INDIGO's bundled-deps Makefile races under parallel make (jpeglib.h consumed before libjpeg installs, libindigocat links before libusb is built, hidapi's configure fires while libtool is still staging elsewhere). The outer PHD2 build still parallelises; only the INDIGO sub-make is serialised. Setting `USE_SYSTEM_LIBINDIGO=ON` opts into the alternate path: `find_package(INDIGO)` (via `cmake_modules/FindINDIGO.cmake`) searches `INDIGO_ROOT` → sibling `../indigo` checkout → `/usr/local` → `/opt/homebrew`, useful for devs who already have a local INDIGO build. The Windows branch is wired as a placeholder that points at `INDIGO_WINDOWS_PORT.md` for the msbuild-based path that fills it in. `HAVE_INDIGO=1` gates the registry defines in `cameras.h`/`scopes.h`/`rotators.h`, so when INDIGO isn't being linked the transport compiles out cleanly. Stub `cam_indigo.cpp` / `scope_indigo.cpp` / `rotator_indigo.cpp` join the source lists; the factories return nullptr until each driver port lands. On macOS the ExternalProject INSTALL_COMMAND copies both `libindigo.dylib` and INDIGO's bundled `libusb-1.0.0.dylib` into `${CMAKE_BINARY_DIR}/libindigo/lib/` and rewrites both install_names to `@loader_path/`, and a POST_BUILD step on the phd2 target drops copies into `OpenAstro PHD2.app/Contents/MacOS/` and re-codesigns. Without that, the .app's launch fails with "Library missing" + sandbox-blocked open when launched from Finder in a Documents/Desktop/Downloads location on macOS Sonoma+.
- Shared INDIGO client base (`src/indigo_client_base.{h,cpp}`): a C++ wrapper that bridges INDIGO's C callback-struct API to virtual-dispatch C++ classes. `IndigoClientBase` owns an `indigo_client` with `client_context = this`, installs static thunks for the bus's `attach`/`define_property`/`update_property`/`delete_property`/`send_message`/`detach` hooks, and dispatches each one through a virtual `On*()` so subclasses (`CameraINDIGO`, `ScopeINDIGO`, `RotatorINDIGO`) override only what they need — analogous to how the INDI path inherits from `INDI::BaseClient`. `IndigoBusGuard` refcounts `indigo_start()`/`indigo_stop()` since the bus is process-global and multiple drivers (camera + scope + rotator on the same profile) must share one start/stop pair. Per-driver property helpers (set switch, wait for state, BLOB receive) will accrete on this base as each driver port lands.
- INDIGO setup dialog (`src/config_indigo.{h,cpp}`). Minimal `wxDialog` with text fields for host (defaults to `localhost`), port (`wxSpinCtrl`, defaults to `7624`), and device name. Each of `CameraINDIGO`, `ScopeINDIGO`, and `RotatorINDIGO` gained a `Setup()` helper that pops the dialog, persists the result to `/indigo/host`, `/indigo/port`, and `/indigo/{camera,mount,rotator}`, and refreshes its displayed `Name`. `Setup()` fires from `Connect()` when no device name is configured (so first-time users see the dialog automatically) and from `ShowPropertyDialog()` / `SetupDialog()` (so users can edit settings without disconnecting). The camera sets `PropertyDialogType = PROPDLG_ANY` to enable the property button in both connected and disconnected states. mDNS server discovery (parallel to `indi_discovery.cpp`) and an INDIGO property tree browser (parallel to `indi_gui.cpp`) are deliberately out of scope for this commit — users type the device name as it appears in `indigo_server` logs.
- INDIGO camera driver. Multi-inherits `GuideCamera` + `IndigoClientBase`. Connect drives the device-side handshake via `CONNECTION`; capture writes `CCD_EXPOSURE.EXPOSURE` (seconds) and blocks on a condition variable until the bus delivers a `CCD_IMAGE` BLOB. The driver forces `CCD_IMAGE_FORMAT.FITS` (because we parse FITS on the receiving end) and calls `indigo_enable_blob(ALSO)` when `CCD_IMAGE` is first defined so the server actually streams BLOB bytes instead of the default URL-only mode. Captured BLOBs are parsed with `fits_open_memfile` (cfitsio in-memory) and stored into `usImage`. `GetDevicePixelSize()` reports the pixel pitch from `CCD_INFO.PIXEL_SIZE`. Subframe, hardware binning, gain, and cooling are deliberately out of scope for this first commit — the goal is to land the BLOB plumbing in a reviewable chunk and accrete the rest on top. Factory routing in `camera.cpp` follows the same `StartsWith("INDIGO Camera")` pattern the recent ASCOM-Windows landing introduced for the existing transports.
- INDIGO mount driver. Multi-inherits `Scope` + `IndigoClientBase` like the rotator port. Implements the minimum viable mount surface for guiding: pulse-guide via INDIGO's `GUIDER_GUIDE_RA` / `GUIDER_GUIDE_DEC` properties (EAST/WEST/NORTH/SOUTH items, duration in ms), live RA/Dec read from `MOUNT_EQUATORIAL_COORDINATES`, and `CanPulseGuide` / `CanReportPosition` reporting true once those properties have been seen. `Guide()` writes the pulse duration to the right item and waits on a condition variable for the property state to leave `BUSY` — same blocking shape PHD2 expects from every mount transport. RA/Dec are stored as `std::atomic<double>` so `GetCoordinates()` (called every guide frame from the main thread) is lock-free. Configuration reads `/indigo/host`, `/indigo/port`, `/indigo/mount`. Slew, abort, side-of-pier, guide-rate read, and site lat/long are deliberately not in this commit — the guiding hot path is what PHD2 actually needs and shipping it standalone keeps the diff reviewable; the rest accrete in follow-ups. Factory routing in `scope.cpp` adds an INDIGO entry to `MountList()` and dispatches through an explicit `StartsWith("INDIGO")` branch placed before the `Contains("INDI")` branch so "INDIGO Mount" doesn't get misrouted to the INDI factory.
- INDIGO rotator driver. First real consumer of the INDIGO client wrapper. `RotatorINDIGO` (multi-inherits `Rotator` + `IndigoClientBase`) connects to `indigo_server` over TCP, watches the configured device's `CONNECTION` switch and `ROTATOR_POSITION` number, drives the device-side connect handshake via `indigo_device_connect`, and exposes the live position through `Rotator::Position()`. Configuration reads `/indigo/host`, `/indigo/port`, `/indigo/rotator` (mirroring the `/indi/...` keys). The connect path waits in a `ConnectRotatorInBg` worker so the wx main loop keeps pumping during the 30s timeout — same UX pattern as the INDI rotator. `IndigoClientBase` grew the minimum property-set surface to drive this (`SetSwitch1`, `SetNumber1`, `SetText1`, `ConnectDevice`, `DisconnectDevice`, `FindItem`). The factory routing in `rotator.cpp` adds an INDIGO entry to `RotatorList()` and dispatches it through an explicit `StartsWith("INDIGO")` branch placed **before** the existing `Contains("INDI")` branch — without that ordering an "INDIGO Rotator" choice would have been routed to the INDI factory because "INDIGO" contains "INDI".
