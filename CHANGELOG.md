...
- INDIGO build wiring: new `cmake_modules/FindINDIGO.cmake` locates the libindigo client library + headers (search order: `INDIGO_ROOT` var/env → sibling `../indigo` checkout → `/usr/local` + `/opt/homebrew`). `thirdparty/thirdparty.cmake` calls `find_package(INDIGO QUIET)` and, when found, defines `HAVE_INDIGO=1` so the registry gates in `cameras.h`/`scopes.h`/`rotators.h` enable the INDIGO transport at compile time. When libindigo isn't present the build silently compiles out the INDIGO paths, mirroring how ASCOM behaves on non-Windows. Stub `cam_indigo.cpp` / `scope_indigo.cpp` / `rotator_indigo.cpp` are added to the source lists; the factories return nullptr until the real client wrapper lands.
